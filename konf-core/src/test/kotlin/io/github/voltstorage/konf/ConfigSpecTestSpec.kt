...
object ConfigSpecTestSpec : Spek({
    given("a configSpec") {
        ...
        val config = Config.Companion { addSpec(specForLazy) }
        ...
        on("add repeated item") {
            val spec = ConfigSpec()
            val item by Spec.Companion.dummy.required<Int>()
            spec.addItem(item)
        ...
        group("get operation") {
            ...
            on("get a valid path") {
                ...
                it("should return a config spec with the proper items and inner specs") {
                    ...
                    assertThat(
                        spec["a.bb.inner3"].innerSpecs.toList()[0].prefix,
                        equalTo("a"),
                    )
                    assertThat(
                        spec["a.bb.inner3"].innerSpecs.toList()[1].prefix,
                        equalTo("b"),
                    )
                }
            }
            on("get an invalid path") {
                it("should throw NoSuchPathException") {
                    assertThat(
                        { spec["b"] },
                        throws(has(NoSuchPathException::path, equalTo("b"))),
                    )
                    assertThat({ spec["a."] }, throws<InvalidPathException>())
                    assertThat(
                        { spec["a.b"] },
                        throws(has(NoSuchPathException::path, equalTo("a.b"))),
                    )
                    assertThat(
                        {
                            spec["a.bb.inner4"]
        ...
        group("prefix operation") {
            ...
            on("prefix with a non-empty path") {
                ...
                it("should return a config spec with the same items and inner specs") {
                    ...
                    assertThat(
                        (Prefix("c") + spec).innerSpecs,
                        equalTo(spec.innerSpecs),
                    )
                }
            }
        }
        ...
        group("prefix inference") {
            ...
            on("companion object of a class") {
                it("should inference prefix as the class name") {
                    assertThat(OriginalSpec.Companion.prefix, equalTo("original"))
                }
            }
        }
    }
