...
import io.github.voltstorage.konf.source.toValue
...
object AdHocConfigItemSpec : Spek({
    ...
    on("cast config to config class property") {
        val networkBufferForCast: NetworkBufferForCast by Config.Companion().withSource(source).cast()
        it("should load correct values") {
    ...
    on("cast config to config class") {
        val networkBufferForCast = Config.Companion().withSource(source).toValue<NetworkBufferForCast>()
        it("should load correct values") {
            assertThat(networkBufferForCast.size, equalTo(1))
            assertThat(networkBufferForCast.maxSize, equalTo(2))
            assertThat(networkBufferForCast.name, equalTo("buffer"))
    ...
    on("cast multi-layer config to config class") {
        val networkBufferForCast = Config.Companion().withSource(source).from.json.string("").toValue<NetworkBufferForCast>()
        it("should load correct values") {
            assertThat(networkBufferForCast.size, equalTo(1))
            assertThat(networkBufferForCast.maxSize, equalTo(2))
            assertThat(networkBufferForCast.name, equalTo("buffer"))
    ...
    on("cast config with merged source to config class") {
        val networkBufferForCast = Config.Companion().withSource(source + Source.from.json.string("")).toValue<NetworkBufferForCast>()
        it("should load correct values") {
            assertThat(networkBufferForCast.size, equalTo(1))
            assertThat(networkBufferForCast.maxSize, equalTo(2))
            assertThat(networkBufferForCast.name, equalTo("buffer"))
