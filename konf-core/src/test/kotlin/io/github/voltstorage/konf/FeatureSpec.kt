...
import io.github.voltstorage.konf.source.UnknownPathsException
    ...
    given("a config") {
        on("the feature is disabled") {
            ...
            it("should ignore unknown paths") {
                val conf =
                    config.from.disabled(Feature.FAIL_ON_UNKNOWN_PATH).json.string(
                        source,
                    )
        ...
        on("the feature is enabled on source") {
            ...
            it("should throws UnknownPathsException and reports the unknown paths") {
                assertThat(
                    {
                        config.from.enabled(Feature.FAIL_ON_UNKNOWN_PATH).json.string(
                            source,
                        )
                    },
                    throws(
...
object LoadKeysCaseInsensitivelySpec : Spek({
    given("a config") {
        on("by default") {
            ...
            val config = Config.Companion().withSource(source)
        ...
        on("the feature is disabled") {
            val source =
                mapOf("somekey" to "value").asSource().disabled(
        ...
        on("the feature is enabled on config") {
            val source = mapOf("somekey" to "value").asSource()
            val config =
                Config.Companion().enable(Feature.LOAD_KEYS_CASE_INSENSITIVELY).withSource(
                    source,
                )
        ...
        on("the feature is enabled on source") {
            val source =
                mapOf("somekey" to "value").asSource().enabled(
                    Feature.LOAD_KEYS_CASE_INSENSITIVELY,
                )
            val config = Config.Companion().withSource(source)
            it("should load keys case-insensitively") {
...
object LoadKeysAsLittleCamelCaseSpec : Spek({
    given("a config") {
        ...
        on("the feature is enabled") {
            val source =
                mapOf("some_key" to "value").asSource().enabled(
                    Feature.LOAD_KEYS_AS_LITTLE_CAMEL_CASE,
                )
            val config = Config.Companion().withSource(source)
            it("should load keys as little camel case") {
                val someKey by config.required<String>()
        ...
        on("the feature is disabled on config") {
            ...
            val config =
                Config.Companion().disable(Feature.LOAD_KEYS_AS_LITTLE_CAMEL_CASE).withSource(
                    source,
                )
        ...
        on("the feature is disabled on source") {
            val source =
                mapOf("some_key" to "value").asSource().disabled(
                    Feature.LOAD_KEYS_AS_LITTLE_CAMEL_CASE,
                )
            val config = Config.Companion().withSource(source)
            it("should load keys as usual") {
...
object OptionalSourceByDefautSpec : Spek({
    given("a config") {
        on("the feature is disabled") {
            val config = Config.Companion().disable(Feature.OPTIONAL_SOURCE_BY_DEFAULT)
            it("should throw exception when file is not existed") {
                assertThrows<FileNotFoundException> { config.from.file("not_existed.json") }
            }
        }
        on("the feature is enabled on config") {
            val config = Config.Companion().enable(Feature.OPTIONAL_SOURCE_BY_DEFAULT)
